...
def _clean(s: str) -> str:
    # Recover bytes mangled into lone surrogates (UTF-8 read under a C locale) so eth_abi
    # can encode them. No-op for clean ASCII/UTF-8 input.
    try:
...
def build_persistent_request_input(
    ...
    model = _clean(os.getenv("MODEL", default_model_for_provider(provider_name)))
